        ...
        for col in range(COLUMNS):
            if available_spots(row, col):
                ...
                if (score > highestScore):
                    ...
                    bestMove_X = row
                    bestMove_Y = col
    if available_spots(bestMove_X, bestMove_Y):
        board[bestMove_X][bestMove_Y] = 2
    return


# minimax algorithem function
def minimax(board, isMaximizing):
    if check_winner_no_drawing(1):
        return -100
    elif check_winner_no_drawing(2):
        return 100
...
def draw_x_o():
    for r in range(ROWS):
        for c in range(COLUMNS):
            if board[r][c] == 1:
                pg.draw.circle(screen, FIGURES, (int(c * 200 + 200 / 2), int(r * 200 + 200 / 2)), FIGURE_C_RADIUS,
                               FIGURE_C_WIDTH)  # multiply by 200 to ge the respective corrdinates in the gui and we draw them in the center of the gui
            elif board[r][c] == 2:
                pg.draw.line(screen, X_COLOR, (c * 200 + SPACE, r * 200 + 200 - SPACE),
                             (c * 200 + 200 - SPACE, r * 200 + SPACE), X_WiDTH)
                pg.draw.line(screen, X_COLOR, (c * 200 + SPACE, r * 200 + SPACE),
                             (c * 200 + 200 - SPACE, r * 200 + 200 - SPACE), X_WiDTH)
...
def check_winner_no_drawing(player):
    # vertical win
    for col in range(COLUMNS):
        if board[0][col] == player and board[1][col] == player and board[2][col] == player:
            return True
